...
MONGO_URL = 'mongodb://' + \
    config.read()['MONGO_USER'] + ':' + \
    config.read()['MONGO_PASS'] + '@' + \
    config.read()['MONGO_HOST'] + ':' + \
    config.read()['MONGO_PORT'] + '/?authSource=' + \
    config.read()['MONGO_AUTHDB']
...
db = client[config.read()['MONGO_DB']]
